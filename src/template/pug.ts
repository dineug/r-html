...
const createTagged =
  ...
  (strings: TemplateStringsArray, ...values: any[]): TemplateLiterals => {
    ...
    const tpl = strings
      ...
      .join('');
    const node = createTNode(pugParser(tpl));

    templateCache.set(strings, { node });
    return templateLiterals;
  };

export const pug = createTagged(TemplateLiteralsType.pug);
export const pugSVG = createTagged(TemplateLiteralsType.pugSVG);
